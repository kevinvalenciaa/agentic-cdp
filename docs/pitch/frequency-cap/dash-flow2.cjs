...
pre{margin:0;white-space:pre-wrap}.k{color:#7fb0ff}.s{color:#f1b862}.n{color:#6fd7ad}.hl{background:rgba(111,215,173,.18);border-radius:3px;padding:2px 6px;margin:-2px -6px}</style>
...
(async () => {
  ...
  // wait for the launch job
  for (let i = 0; i < 40; i++) {
    await p.goto("http://localhost:3000/launched", { waitUntil: "networkidle" });
    const t = (await p.innerText("main").catch(() => "")).replace(/\s+/g, " ");
    if (!/Nothing launched yet/.test(t)) { console.log("launched:", t.slice(0, 700)); break; }
    await p.waitForTimeout(3000);
  }
  await shot("dash-05-launched", true);
  await p.goto("http://localhost:3000/opportunities", { waitUntil: "networkidle" });
  await p.click("text=Second-purchase SMS to one-time buyers"); await p.waitForTimeout(1200);
  await p.click("[role=tab]:has-text('Plan')"); await p.waitForTimeout(1000);
  console.log("plan:", (await p.innerText("[role=dialog]").catch(() => "")).replace(/\s+/g, " ").slice(0, 900));
  await shot("dash-03-sms-opportunity-plan");
  const dlg = p.locator("[role=dialog]");
  await dlg.evaluate(el => { const s = el.querySelector("[data-radix-scroll-area-viewport], .overflow-y-auto, .overflow-auto"); if (s) s.scrollTop = 600; });
  await shot("dash-03b-sms-opportunity-plan-scrolled");
